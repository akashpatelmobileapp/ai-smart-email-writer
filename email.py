...
def get_chain_and_agent():
    ...
    def search_attachments(topic: str) -> str:
        ...
        try:
            ...
            return f"Attachment search is unavailable right now ({type(e).__name__}). Please try again later."
        ...
        for i, r in enumerate(results):
            url     = r.get("url", "")
            snippet = r.get("content", "")[:130]
            out.append(f"{i+1}. {r.get('title', 'Untitled')}\n   {snippet}...\n   {url}")
        return "\n\n".join(out)
...
Only call the tool when explicitly asked."""),
                    ...
                    st.error(f"Couldn't fetch attachment suggestions: {e}")
                    st.stop()
            st.rerun()
